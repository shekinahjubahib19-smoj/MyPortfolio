    ...
    if (loginEl && heroEl) {
      ...
      const getMaxTransitionMs = (el) => {
        try {
          ...
          // transitionDuration can be comma-separated; take max
      ...
      if (loginTransitionMs === 0 && heroTransitionMs === 0) {
        // no transitions — hide immediately
        setShowLogin(false);
        setIsLeaving(false);
      ...
      // Fallback: if transitionend doesn't fire, ensure we hide after max duration
      ...
      // If elements aren't found, fallback to timeouts
      const finalDelay = Math.max(loginDuration, heroDelay + heroDuration);
      setTimeout(() => {
        setShowLogin(false);
        setIsLeaving(false);
        setIsHeroLeaving(false);

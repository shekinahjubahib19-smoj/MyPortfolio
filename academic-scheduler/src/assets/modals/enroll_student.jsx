...
const EnrollStudent = ({ isOpen, onClose, onSaved, initialData }) => {
  ...
  const [level, setLevel] = useState(() => initialData?.current_level || 'Level 1');
  const [status, setStatus] = useState(() => initialData?.enrollment_status || 'Active');
  ...
  return (
    <div className="registration-overlay" role="dialog" aria-modal="true" onClick={onClose}>
      <div className="registration-modal" onClick={stop} style={{ width: 'min(90vw, 620px)' }}>
        ...
        <h3 style={{ marginTop: 0, textAlign: 'center' }}>{initialData?.id ? 'Edit Student' : 'Enroll Student'}</h3>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <label>Student Code</label>
          <input className="input-field" value={studentCode} onChange={(e) => setStudentCode(e.target.value)} />
          ...
          <div className="actions">
            ...
            <button type="submit" className="btn primary" disabled={working}>{working ? 'Saving…' : (initialData?.id ? 'Update' : 'Enroll')}</button>
          </div>
          {message && <p style={{ fontSize: '0.85rem', color: message.startsWith('Saved') ? '#47d147' : '#ff4d4d' }}>{message}</p>}
        </form>
      </div>
    </div>
  );
